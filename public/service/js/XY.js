...
// ====== 以下是原始代碼 ======
    ...
    function handleLineLogin() {
        ...
        if (currentOrigin !== CANONICAL_ORIGIN) {
            console.log(`🔄 [XY] 重定向到正式域名: ${CANONICAL_ORIGIN}`);
    ...
    function createApplicantCard(name = '', canRemove = true) {
        ...
        card.innerHTML = `
            ...
            <div class="applicant-details">
                ...
                <div class="form-group">
                    <label for="jingzhi-${cardId}">1. 金紙（$100 / 份）</label>
                    <div style="display: flex; align-items: center; gap: 10px;">
                        <input type="number" id="jingzhi-${cardId}" class="input-field donation-item" data-price="100" value="0" min="0" placeholder="請輸入數量">
                        <span style="color: #666;">份</span>
                    </div>
                </div>
            
                <div class="form-group">
                    <label for="suixi-${cardId}">2. 隨喜功德</label>
                    <div style="display: flex; align-items: center; gap: 10px;">
                        <span style="color: #666;">NT$</span>
                        <input type="number" id="suixi-${cardId}" class="input-field donation-item" data-price="1" value="0" min="0" step="100" placeholder="請輸入金額">
                    </div>
                </div>
    ...
    async function handleSubmit() {
        ...
        try {
            ...
            cards.forEach(card => {
                const donationItems = card.querySelectorAll('.donation-item');
                const jingzhi = parseInt(donationItems[0].value) || 0;
                const suixi = parseInt(donationItems[1].value) || 0;
