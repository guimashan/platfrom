...
- ✅ 生辰欄位改為三個獨立輸入框（年 / 月 / 日），搭配完整的日期選擇器
  - 年欄位：52px，4 位數（____）
  - 月欄位：34px，2 位數（__）
  - 日欄位：34px，2 位數（__）
  - 日期選擇器：flex: 1，min-width: 120px，附提示文字「或點此使用日期選擇器」
  - 單行布局設計（桌面和手機一致）
  - 雙向同步：手動輸入或日期選擇器任一方式都會同步
  - 自動限制只能輸入數字
  - 緊湊型設計：縮小間距、padding、字體以適應單行
  - 注意：手機螢幕（320-375px）可能出現輕微橫向滾動（用戶已知悉並接受）
